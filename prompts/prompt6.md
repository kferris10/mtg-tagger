...
- anthem: Persistent static or triggered effect that increases the power or toughness of multiple creatures you control (e.g., Glorious Anthem, Intangible Virtue). Does not need to affect ALL creatures â€” tribal lords and type-restricted buffs qualify. Does not include temporary combat-only buffs (see overrun).
- overrun: Temporary, combat-focused burst that grants power, toughness, or evasion to multiple creatures for a turn, enabling a lethal attack (e.g., Overrun, Craterhoof Behemoth, Triumph of the Hordes). Distinguished from anthem by being a one-shot effect rather than a persistent static ability. Does NOT include effects that pump attacking creatures based on mana paid or number of attackers (e.g., Jazal Goldmane) â€” those are mana_sink.
- go_tall: Permanently or persistently increases a single creature's power or toughness numbers (e.g., Blackblade Reforged, Bear Umbra, Lion Sash, Tuvasa the Sunlit). Includes auras and equipment that give a static +X/+X boost to one creature, and cards that scale a single creature's size based on game state. Does NOT include buffs that apply to all creatures of a type (see anthem). Does NOT include temporary one-turn combat buffs. Does NOT include defensive keywords like indestructible or hexproof â€” cards that grant those are protection, not go_tall.
...
- get_through: A creature has, or an effect grants, evasion or trample allowing a creature to deal combat damage despite blockers (e.g., flying, trample, shadow, menace, intimidate, unblockable). Tag a creature card if it naturally has one of these keywords. Tag an equipment or aura if it grants one of these keywords. Also includes effects like Mirri, Weatherlight Duelist that restrict how many creatures can block. Does NOT include anthem-style effects that give +1/+1 to all flying creatures â€” those are anthem even if the card also has flying. Does NOT tag go_wide cards as get_through just because the tokens they create have flying or trample â€” the card's purpose is token generation. Does NOT include double strike or first strike â€” those are combat damage bonuses, not evasion.
- protection: Tag protection ONLY when a card grants or has the specific defensive keywords: indestructible, hexproof, shroud, ward, regenerate, or protection from [quality] â€” directly on the permanent or via an equipment/aura. This is about mechanical immunity to targeting or destruction, not general defensive utility. Examples: Swiftfoot Boots, Lightning Greaves, Darksteel Plate, Fleecemane Lion (hexproof/indestructible when monstrous), Jareth Leonine Titan (protection from chosen color). Do NOT tag protection for blink/flicker effects â€” Cloudshift, Ephemerate, Momentary Blink, and Whitemane Lion do NOT qualify even though they can dodge removal; tag those as blink_flicker instead. Do NOT tag protection for combat keywords like first strike, vigilance, trample, or deathtouch.
- etb_effects: Triggers or abilities that fire specifically when the card enters the battlefield, generating an additional effect beyond the card's passive existence (e.g., Whitemane Lion returns a creature, Guardian of Ghirapur exiles a creature). Do not tag a card solely because it is a permanent that enters the battlefield. Do NOT use etb_effects if the ETB is already fully captured by another mechanic â€” Solemn Simulacrum's ETB is ramp; Reclamation Sage's ETB is targeted_disruption; Mulldrifter's ETB is card_advantage. When in doubt: if all of the card's notable functions are already tagged under ramp, card_advantage, targeted_disruption, mass_disruption, go_wide, or blink_flicker, do NOT also add etb_effects. Reserve etb_effects for meaningful ETB value not described by any other mechanic.
...
- **B-Tier**: Good effects (Mind Stone ramp, Felidar Cub targeted_disruption, Sign in Blood card_advantage, Swiftfoot Boots protection, Conjurer's Closet blink_flicker, Bear Umbra go_tall)
- **C-Tier**: Moderate effects (Mulldrifter card_advantage, Whispersilk Cloak get_through, Ephemerate blink_flicker)
- **D-Tier**: Weak effects

Additional tier anchors for new mechanics:
- get_through: S-Tier = Embercleave (grants trample+double strike to a large attacker). A-Tier = Trailblazer's Boots (near-unblockable). B-Tier = Whispersilk Cloak (unblockable + shroud). C-Tier = a single creature with flying as an incidental keyword.
- protection: A-Tier = Darksteel Plate (indestructible). B-Tier = Swiftfoot Boots (hexproof + haste), Lightning Greaves (shroud + haste). C-Tier = totem armor aura.
- go_tall: A-Tier = Blackblade Reforged (massive scaling buff). B-Tier = Bear Umbra (+2/+2 + untap). C-Tier = minor +1/+1 aura or equipment.
- mana_sink: A-Tier = Staff of Domination (infinite mana outlet). B-Tier = Jazal Goldmane, Leafdrake Roost. C-Tier = single X-cost activated ability.
- blink_flicker: A-Tier = Teleportation Circle (repeatable every turn). B-Tier = Conjurer's Closet (end of turn). C-Tier = Ephemerate (one-shot, rebound).
- anthem: A-Tier = Craterhoof Behemoth (massive one-turn buff, ETB). B-Tier = Glorious Anthem (permanent +1/+1 all). C-Tier = tribal lord with minor buff.
- overrun: S-Tier = Craterhoof Behemoth. A-Tier = Overrun, Triumph of the Hordes. B-Tier = smaller one-turn pump spell.
- untap_effects: S-Tier = Seedborn Muse (untap all on each opponent's turn). A-Tier = Wilderness Reclamation. B-Tier = Nature's Chosen (single untap, repeatable). C-Tier = one-time untap effect.
</card_tier>
...
3. âœ“ Mechanic name spelling: the evasion mechanic is "get_through" (NOT "go_through") â€” double-check this
4. âœ“ Used exact tier names (S+ Tier, S-Tier, A-Tier, B-Tier, C-Tier, D-Tier)
5. âœ“ Tagged ALL applicable mechanics - cards frequently have more than one mechanic
6. âœ“ Followed the instructions in <mechanics> and <card_tier> when evaluating mechanics and tiers.
7. âœ“ Considered both immediate and ongoing effects
8. âœ“ Evaluated multiplayer impact for tier ratings
9. âœ“ Ignored mana cost when assigning tier ratings
10. âœ“ Rated each mechanic based solely on that specific effect, ignoring other abilities on the card
11. âœ“ Did NOT tag blink/flicker spells (Cloudshift, Ephemerate, Whitemane Lion) as protection â€” those are blink_flicker
12. âœ“ Did NOT tag go_tall for cards that grant indestructible or hexproof â€” those are protection
13. âœ“ Did NOT tag get_through for go_wide cards whose tokens happen to have flying or trample
14. âœ“ Did NOT tag etb_effects if the ETB is already fully covered by ramp, card_advantage, targeted_disruption, mass_disruption, go_wide, or blink_flicker
15. âœ“ Return only the JSON output, no additional text or explanations
</quality_control>
