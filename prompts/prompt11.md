...
- card_advantage: Net positive card advantage giving you access to 1+ more cards than you spent to cast (Harmonize, Rhystic Study, Mulldrifter). Does not include cantrips, cycling, card selection, or tutors unles
- targeted_disruption: Removes, disables, or meaningfully interacts with a SINGLE target opponent card (Path to Exile, Counterspell). Includes targeted removal, bounce, tap effects, counterspells, dealing damage to a creature, forcing a fight, stealing, stealing temporarily, or any effect that reduces the impact of a specific opponent permanent. Minor effects still qualify — deal 1 damage to a target creature or tap a single creature with no other consequence = D-Tier. Err toward tagging over skipping for unusual or minor targeting effects.
- mass_disruption: Affects MULTIPLE opponent cards or opponents simultaneously (Wrath of God, Rest in Peace). Includes mass removal, mass bounce, graveyard hate, tapping multiple opponent creatures, dealing damage to all creatures or all opponents, taxing effects that restrict opponent actions globally (Propaganda). Cards with both modes qualify for both targeted_disruption and mass_disruption. Err toward tagging over skipping for unusual or minor board-wide effects.
...
- C-Tier: Moderate (Mulldrifter card_advantage, Whispersilk Cloak get_through, Ephemerate blink_flicker, minor +1/+1 aura go_tall, totem armor protection, tap all creatures opponents control mass_disruption)
- D-Tier: Weak or marginal (deal 1 damage to target creature targeted_disruption, tap a single creature with no other effect targeted_disruption, minor global ping or tap effect mass_disruption)
...
- DID tag unusual disruption effects (damage to a creature, tapping, minor ping) as targeted_disruption or mass_disruption, even when the effect is small — use D-Tier if weak, not zero tags
