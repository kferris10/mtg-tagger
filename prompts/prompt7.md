...
- overrun: ONE-SHOT combat burst granting power, toughness, or evasion to multiple creatures for one attack (Overrun, Craterhoof Behemoth). Distinguished from anthem by being temporary. Does NOT include effects that pump attackers based on mana paid or attacker count â€” those are mana_sink.
- go_tall: Permanently or persistently increases a SINGLE creature's power or toughness numbers (Blackblade Reforged, Bear Umbra, Tuvasa the Sunlit). Includes auras and equipment with a static +X/+X boost or scaling growth. Does NOT include type-wide buffs (see anthem). Does NOT include indestructible or hexproof â€” those are protection, not go_tall.
...
Step 1: For each card, recall the full oracle text and identify ALL abilities â€” primary, secondary, and triggered.
...
- Used ONLY mechanic names from [Mechanics] â€” the evasion mechanic is "get_through" (NOT "go_through")
- Did NOT tag blink/flicker spells as protection
- Did NOT tag go_tall for cards that grant indestructible/hexproof â€” use protection
